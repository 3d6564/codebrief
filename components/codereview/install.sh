...
--agent is required with --yes. Without --yes, an interactive terminal can
choose an agent if --agent is omitted.

Agents:
  opencode   OpenCode primary agent and /codereview command
  prompt     Canonical prompt file for any other tool
  claude     Claude Code agent and /codereview skill
  cursor     Cursor /codereview skill
...
Options:
  ...
  --yes, -y              Replace existing Codefactory files without prompting.
...
PROMPT_SOURCE="$SCRIPT_DIR/prompt/codereview.md"
PACKAGING="$SCRIPT_DIR/packaging"

if [[ ! -f "$PROMPT_SOURCE" ]]; then
    echo "Codefactory prompt is missing: $PROMPT_SOURCE" >&2
    exit 1
fi
...
confirm_replace() {
    ...
    read -rp "Replace existing Codefactory files? [y/N] " reply
    case "$reply" in
...
refuse_competing() {
    ...
    if [[ -e "$path" || -L "$path" ]]; then
        echo "Competing Codefactory definition found: $path" >&2
        echo "Move or remove it before installing Codefactory." >&2
        exit 1
    fi
}
...
install_opencode() {
    ...
    dest_agent="$root/agents/codereview.md"
    dest_command="$root/commands/codereview.md"
    refuse_competing "$root/agent/codereview.md"
...
install_prompt() {
    local dest
    if [[ "$MODE" == "global" ]]; then
        dest="$HOME/.config/codefactory/codereview.md"
    else
        dest="$TARGET_DIR/.codefactory/codereview.md"
    fi
...
install_claude() {
    ...
    if [[ "$MODE" == "global" ]]; then
        agent_dest="$HOME/.claude/agents/codereview.md"
        skill_dest="$HOME/.claude/skills/codereview/SKILL.md"
    else
        agent_dest="$TARGET_DIR/.claude/agents/codereview.md"
        skill_dest="$TARGET_DIR/.claude/skills/codereview/SKILL.md"
    fi
...
install_cursor() {
    local dest
    if [[ "$MODE" == "global" ]]; then
        dest="$HOME/.cursor/skills/codereview/SKILL.md"
    else
...
install_copilot() {
    local dest
    if [[ "$MODE" == "global" ]]; then
        dest="$HOME/.copilot/agents/codereview.agent.md"
    else
        dest="$TARGET_DIR/.github/agents/codereview.agent.md"
    fi
    write_composed_markdown "$PACKAGING/copilot/agent-frontmatter.md" "$dest"
    echo "Select the codereview custom agent in Copilot chat or the agents dropdown."
}

install_codex() {
    local dest
    if [[ "$MODE" == "global" ]]; then
        dest="$HOME/.codex/agents/codereview.toml"
    else
        dest="$TARGET_DIR/.codex/agents/codereview.toml"
    fi
    write_codex_toml "$dest"
    echo "Ask Codex to use the codereview custom agent, or spawn it by name."
}
